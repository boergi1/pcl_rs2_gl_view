...
Rs2_PCL_Converter::Rs2_PCL_Converter(DeviceInterface *in_interface_ref, ProcessingInterface *out_interface_ref, std::vector<CameraType_t> camera_types)
{
    ...
    const cv::Mat R_identity = cv::Mat::eye(3,3, CV_32F);
      ...
      //  std::cerr << "DEBUG Converter" << std::endl;
        ...
        for (size_t i = 0; i < m_ref_to_depth_queues->size(); i++)
        {
            ...
            if (!queue_depth->isEmpty())
            {
                ...
                cv::Mat depth_mat = cv::Mat(cv::Size(depth_frame.get_width(), depth_frame.get_height()), CV_16UC1,
                ...
                //                std::cout << "DEBUG intrinsics for camera " << queue_depth->getCameraType() << std::endl
                //                          << "fx " << queue_depth->getIntrinsics()->fx << " fy " << queue_depth->getIntrinsics()->fy
                //                          << " cx " << queue_depth->getIntrinsics()->cx << " cy " << queue_depth->getIntrinsics()->cy << std::endl;


                //                std::cout << "DEBUG extrinsics for camera " << queue_depth->getCameraType() << std::endl
                //                          << queue_depth->getExtrinsics()->R[0] << " " << queue_depth->getExtrinsics()->R[1] << " " << queue_depth->getExtrinsics()->R[2] << " | " << queue_depth->getExtrinsics()->T[0] << std::endl
                //                          << queue_depth->getExtrinsics()->R[3] << " " << queue_depth->getExtrinsics()->R[4] << " " << queue_depth->getExtrinsics()->R[5] << " | " << queue_depth->getExtrinsics()->T[1] << std::endl
                //                          << queue_depth->getExtrinsics()->R[6] << " " << queue_depth->getExtrinsics()->R[7] << " " << queue_depth->getExtrinsics()->R[8] << " | " << queue_depth->getExtrinsics()->T[2] <<  std::endl;

                // Depth to 3D Cloud task
                //                FrameToCloudTask* task_f2c = new FrameToCloudTask(queue_depth->getIntrinsics(), queue_depth->getExtrinsics());
                        ...
                        for(auto& cloudqueue : *m_ref_to_cloud_queues)
                        {
                            if ( taskid == cloudqueue->getCameraType())
                            {
                                ...
                                std::cout << "(Converter) Added DepthCloud to queue " << taskid << " ("
                                          << std::get<0>(cloudtuple) << ") size: " << std::get<1>(cloudtuple)->size() << std::endl;
#endif
                            }
                        }
                    }
...
#endif
            std::this_thread::sleep_for(std::chrono::nanoseconds(DELAY_CONVERTER_NS));
        }
    }
}
